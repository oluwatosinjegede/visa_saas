...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadHealth() {
      try {
        ...
        setModules(Array.isArray(health) ? health : [])
        setStatus('ready')
      } catch (error) {
  ...
  const healthyCount = useMemo(() => {
    return modules.filter((item) => item?.ok).length
  }, [modules])
    ...
    <main className="homepage">
      ...
      <section className="section system-status">
        <div className="status-head">
          ...
          <p>
            Backend connectivity: <strong>{healthyCount}</strong> /{' '}
            <strong>{modules.length}</strong> services healthy
          </p>
        </div>
        ...
        {status === 'error' && (
          <p className="status-msg error">
            Unable to load backend health status. Homepage content is still available.
          </p>
        )}
